...
mutationalSignatureAnalysis = function(table, genome_build = 'GRCh38') {
	...
	table_subset = table[!grepl(regexPatterns$snp_identifier, variant_id) & nchar(ref_allele) == 1 & nchar(alt_allele) == 1]
...
parseEpitopePredictions = function(path, sample_table = sample_info, pattern = '_epitopes\\.csv') {
	...
	predictions = lapply(seq(1, length(files)),
											 function(i) {
											 	data = fread(files[i])
											 	data[, sample_prefix := sub(pattern = regexPatterns$seqdata_prefix, replacement = '', x = short_names[i])]
