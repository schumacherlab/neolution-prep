...
prediction_table = merge(x = data.table(patient_id = names(sapply(predictions_by_patient, nrow)),
																				hla_alleles = sapply(names(predictions_by_patient), function(id) {
																					length(unique(dropNa(unlist(sample_info[patient_id == id, grep('hla',names(sample_info), value = TRUE), with = F]))))
																					}),
...
prediction_table[, selected_epitope_yield_rate_per_allele := round(x = selected_epitopes_predicted / non_syn_coding_variants / hla_alleles, digits = 2)]
setcolorder(x = prediction_table,
						neworder = c('patient_id', 'hla_alleles', 'total_epitopes_predicted', 'total_epitope_yield_rate_per_allele', 'selected_epitopes_predicted', 'selected_epitope_yield_rate_per_allele', 
												 setdiff(x = names(prediction_table), y = c('patient_id', 'hla_alleles', 'total_epitopes_predicted', 'total_epitope_yield_rate_per_allele', 'selected_epitopes_predicted', 'selected_epitope_yield_rate_per_allele'))))
```
...
__Total number of selected epitopes: `r sum(prediction_table$selected_epitopes_predicted)`__  
...
all_predictions_unfiltered_by_patient = lapply(all_predictions_unfiltered_by_patient,
																							 function(dt) {
																							 	dt[model_prediction >= runParameters$predictions$model_cutoff, passes_model_cutoff := TRUE]
																							 	dt[model_prediction < runParameters$predictions$model_cutoff, passes_model_cutoff := FALSE]
																							 	
																							 	dt[rna_expression > runParameters$predictions$expression_cutoff | is.na(rna_expression), passes_expression_cutoff := TRUE]
																							 	dt[rna_expression <= runParameters$predictions$expression_cutoff , passes_expression_cutoff := FALSE]
																							 })
